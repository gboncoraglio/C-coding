...
t_prodotto* elenco_prodotti_reparto(t_lista_prodotti lista, int reparto, int *numero_elementi) {
	...
	t_prodotto* vettore = (t_prodotto*)malloc(sizeof(t_prodotto) * contatore);
	if (vettore == NULL)
		return NULL;
	int i = 0;
	lista = aux;
...
int menu(void) {
	...
	5) elenco degli articoli aventi quantita minore di un determinato valore \n\n \
			");
		printf(">>> ");
...
void calcolo_estremi(t_lista_prodotti lista, char * casa_produttrice, int * valore_massimo, int * valore_minimo) {
	*valore_massimo = *valore_minimo = -1;

	while (lista != NULL) {
		if (strcmp(lista->dato.casa_produttrice, casa_produttrice) == 0) {
			*valore_massimo = *valore_minimo = lista->dato.quantita_presente;
			break;
		}
		lista = lista->successivo;
	}
	...
	while (lista != NULL) {
		if (strcmp(lista->dato.casa_produttrice, casa_produttrice) == 0) {
			if (lista->dato.quantita_presente > * valore_massimo)
				*valore_massimo = lista->dato.quantita_presente;
			if (lista->dato.quantita_presente < *valore_minimo)
				*valore_minimo = lista->dato.quantita_presente;
		}
		lista = lista->successivo;
	}
}
...
int main(void) {
	...
	do {
		...
		case 1: // carica da file, funzione aggiunta per caricare i dati per il test più
			// rapidamente
			carica_file("elenco.txt", &elenco_prodotti);
			break;
		case 2:
			// 2) Realizzare una funzione che legga da standard input i dati di un articolo e li inserisca in modo
			// opportuno all’interno della lista degli articoli
			// 1. leggere un record da console
			printf("Inserisci i dati di un prodotto\n");
			printf("\t codice: ");
			fgets(buffer, STRINGA_DIMENSIONE, stdin);
			buffer[strlen(buffer) - 1] = '\0';
			strcpy(un_prodotto.codice, buffer);

			printf("\t nome: ");
			fgets(buffer, STRINGA_DIMENSIONE, stdin);
			buffer[strlen(buffer) - 1] = '\0';
			strcpy(un_prodotto.nome, buffer);

			printf("\t casa produttrice: ");
			fgets(buffer, STRINGA_DIMENSIONE, stdin);
			buffer[strlen(buffer) - 1] = '\0';
			strcpy(un_prodotto.casa_produttrice, buffer);
			...
			printf("\t presso: ");
			fgets(buffer, STRINGA_DIMENSIONE, stdin);
			un_prodotto.prezzo_unitario = atoi(buffer);

			printf("\t quantita presente: ");
			fgets(buffer, STRINGA_DIMENSIONE, stdin);
			un_prodotto.quantita_presente = atoi(buffer);

			// 2. verifica del record
			// 3. inserire nell'archivio
		...
		case 4:
			...
			int numero_elementi;
			t_prodotto* v;
			v = elenco_prodotti_reparto_bis(elenco_prodotti, stringa2reparto(buffer), &numero_elementi);
			if (v != NULL) {
				for (int i = 0; i < numero_elementi; ++i)
					prodotto_stampa(stdout,v[i]);
				free(v);
			}
			break;
		}
