...
coda EliminaCoda(coda c){
    ...
    aux = c.primo;
    if (c.primo==c.ultimo) c.primo=c.ultimo=NULL;
    else c.primo = c.primo->successivo;
    free (aux);
    return c;
}
...
coda inCoda(coda c,paziente p){
    ...
    aux->info=p;
    aux->successivo=NULL;
    if (testCodaVuota(c)) c.primo=c.ultimo=aux;
    else {  c.ultimo->successivo = aux;
            c.ultimo = aux;
    }
    return c;
}
...
void quesito1 (char *nome, coda *archivio){
    ...
    fp = fopen(nome, "r");
    if (fp==NULL) return;
    while(fscanf(fp,"%s%s%s%d%d",p.CF,p.nome,p.cognome,&p.urgenza,&codicemedico)==5)
